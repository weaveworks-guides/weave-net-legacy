...
WEAVEDEMO_HOSTCOUNT=${WEAVEDEMO_HOSTCOUNT:-1}
AWS_AMI=${AWS_AMI:-}
...
type -P "$AWS" >/dev/null 2>&1 && echo "Cross-cloud demo with Weave and Docker on AWS" || { echo "aws not found, exiting"; exit 1; }
...
if [ -z $AWS_AMI ]; then
    ...
    AWS_AMI=$(curl -s http://cloud-images.ubuntu.com/query/trusty/server/released.current.txt | awk "/ebs\tamd64\t$REGION.*paravirtual/ { print \$8 }")
...
do
    ...
    $MY_SSH $SSH_OPTS ubuntu@$i "sudo weave setup"
    $MY_SSH $SSH_OPTS ubuntu@$i "sudo docker pull weaveworks/weave-gs-nginx-apache"
